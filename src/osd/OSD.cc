    ...
    if (pool->is_tier()) {
      ...
      if (base_pool->require_rollback()) {
        if ((iter->op.op != CEPH_OSD_OP_READ) &&
            ...
            (iter->op.op != CEPH_OSD_OP_CACHE_TRY_FLUSH) &&
            (iter->op.op != CEPH_OSD_OP_CACHE_FLUSH) &&
            (iter->op.op != CEPH_OSD_OP_CACHE_EVICT) &&
            ...
            (iter->op.op != CEPH_OSD_OP_NOTIFY) &&
            (iter->op.op != CEPH_OSD_OP_NOTIFY_ACK) &&
